...
def test_login(username="user", password="user1234"):
    ...
    if response.status_code == 200:
        ...
        print_info(f"Full response: {response_data}")  # Debug: print the full response
...
def test_login_admin():
    ...
    print_header("Testing Admin Login")  # Changed for clarity
    login_result = test_login("admin", "admin1234")
...
def test_create_article():
    ...
    import random
    random_num = random.randint(1000, 9999)
    title = f"Test Article via API Script {random_num}"
    ...
    print_info(f"Headers: {headers}")  # Add debug info
    print_info(f"Data: {data}")  # Add debug info
